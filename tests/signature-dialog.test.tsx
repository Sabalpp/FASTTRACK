...
vi.mock("@/components/SignaturePad", async () => {
  ...
  const SignaturePad = React.forwardRef<SignaturePadHandle, { onStrokeCountChange?: (count: number) => void }>(
    function MockSignaturePad({ onStrokeCountChange }, ref) {
      ...
      React.useImperativeHandle(ref, () => ({
        clear: () => publish(0),
        undo: () => publish(Math.max(0, count - 1)),
        isEmpty: () => count === 0,
        exportPng: async () => {
      ...
      return <button type="button" onClick={() => publish(count + 1)}>Add test stroke</button>;
    }
  );
  return { SignaturePad };
});
